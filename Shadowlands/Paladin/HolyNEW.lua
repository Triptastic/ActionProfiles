...
A[3] = function(icon, isMulti)
    ...
    local function DamageRotation(unitID)
		...
		local useCC = Action.InterruptIsValid(target)
			if useCC and A.BlindingLight:IsReady(target) and Unit(target):GetRange() <= 10 and A.BlindingLight:AbsentImun(target, Temp.TotalAndPhysAndCC) and not Unit(target):IsBoss() and not Unit(target):IsDead() and Unit(target):CanInterrupt(true, nil, 20, 85)
                then 
    ...
    local function HealingRotation(unitID) 
        ...
        if DungeonGroup then
            if A.AuraMastery:IsReady(unitID) and Unit(unitID):TimeToDieX(20) <= 5 and HealingEngine.GetBelowHealthPercentUnits(50, 30) >= 3 then
                return A.AuraMastery:Show(icon)
            end    
        end
        
        if RaidGroup then
            if A.AuraMastery:IsReady(unitID) and Unit(unitID):TimeToDieX(20) <= 5 and HealingEngine.GetBelowHealthPercentUnits(50, 30) >= 5 then
                return A.AuraMastery:Show(icon)
            end    
        end    
        ...
        if A.HolyPrism:IsReady(unitID) and not Unit(unitID):IsDead() and A.HolyPrism:PredictHeal(unitID) and Unit(unitID):HealthPercent() <= 90 and Unit(targettarget):GetRange() <= 30 then
        ...
        if ((A.CrusaderStrike:IsReady(targettarget) and Unit(targettarget):GetRange() <= 5 and IsUnitEnemy(targettarget) and Unit(targettarget):HasDeBuffs(A.Cyclone.ID, true) == 0) 
            or (A.CrusaderStrike:IsReady(focustarget) and Unit(focustarget):GetRange() <= 5 and IsUnitEnemy(focustarget)) and Unit(focustarget):HasDeBuffs(A.Cyclone.ID, true) == 0) 
		...
		local useCC = Action.InterruptIsValid(targettarget)
			if useCC and A.BlindingLight:IsReady(targettarget) and Unit(targettarget):GetRange() <= 10 and A.BlindingLight:AbsentImun(targettarget, Temp.TotalAndPhysAndCC) and not Unit(targettarget):IsBoss() and IsUnitFriendly(target) and not Unit(target):IsDead() and Unit(targettarget):CanInterrupt(true, nil, 20, 85)
                then 
                    return A.BlindingLight:Show(icon)       
                end            
        ...
        if ((A.HammerofWrath:IsReady(targettarget) and Unit(targettarget):GetRange() <= 10 and IsUnitEnemy(targettarget) and Unit(targettarget):HasDeBuffs(A.Cyclone.ID, true) == 0) 
			or (A.HammerofWrath:IsReady(focustarget) and Unit(focustarget):GetRange() <= 10 and IsUnitEnemy(focustarget) and Unit(focustarget):HasDeBuffs(A.Cyclone.ID, true) == 0)) 
