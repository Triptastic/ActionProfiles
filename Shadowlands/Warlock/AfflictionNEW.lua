...
Action[ACTION_CONST_WARLOCK_AFFLICTION] = {
    ...
    ShadowEmbrace				= Action.Create({ Type = "Spell", ID = 32388, Hidden = true		}),	
...
A[3] = function(icon, isMulti)
	...
	local function EnemyRotation(unit)	
		...
		local function AoERotation(unit)
			...
			if A.SeedofCorruption:IsReady("target", nil, nil, true) and (not isMoving) and UseAoE and Temp.SeedofCorruptionDelay == 0 and CorruptionRefreshable and Unit("target"):HasDeBuffs(A.SeedofCorruptionDebuff.ID, true) == 0 and Player:AreaTTD(40) > 5 and Unit(player):HealthPercent() >= DrainLifeHP then
				return A.SeedofCorruption:Show(icon)
			end	 	
			...
			if A.VileTaint:IsReady(player, nil, nil, true) and (not isMoving) and not A.IsSpellInCasting(A.VileTaint) and Player:SoulShardsP() > 1 and Player:AreaTTD(40) > 9 and Unit(player):HealthPercent() >= DrainLifeHP then
				return A.VileTaint:Show(icon)
			end			
			...
			if BurstIsON("target") and (Player:GetCovenant() == 4 or Player:GetCovenant() == 1 or Player:GetCovenant() == 0) and A.SummonDarkglare:GetCooldown() < 2 and (Unit(unit):HasDeBuffs(A.PhantomSingularityDebuff.ID, true) > 2 or not A.PhantomSingularity:IsTalentLearned()) then
				if PrepareDarkglare() then
					return true
				end
			end	
			...
			if BurstIsON("target") and Player:GetCovenant() == 3 and (A.SoulRot:IsReady(unit) or Unit(unit):HasDeBuffs(A.SoulRot.ID, true) > 0) and A.SummonDarkglare:GetCooldown() < 2 and (Unit(unit):HasDeBuffs(A.PhantomSingularityDebuff.ID, true) > 2 or not A.PhantomSingularity:IsTalentLearned()) then
				if PrepareDarkglare() then
					return true
				end
			end	
		...
		if MultiUnits:GetActiveEnemies() > 3 then
			if UseAoE and AoERotation() then
				A.Toaster:SpawnByTimer("TripToast", 0, "USING AOE!", "AOE ROTATION!", A.SummonDarkglare.ID) 			
				return true
			end
		end	
		...
		if BurstIsON(unit) and Player:GetCovenant() == 2 and Unit(unit):HasDeBuffs(A.ImpendingCatastrophe.ID, true) > 0 and A.SummonDarkglare:GetCooldown() < 2 and (Unit(unit):HasDeBuffs(A.PhantomSingularityDebuff.ID, true) > 2 or not A.PhantomSingularity:IsTalentLearned()) then
			if PrepareDarkglare() then
				A.Toaster:SpawnByTimer("TripToast", 0, "DARKGLARE!", "PREPPING DARKGLARE", A.SummonDarkglare.ID) 			
				return true
			end
		end
		...
		if BurstIsON(unit) and Player:GetCovenant() == 3 and Unit(unit):HasDeBuffs(A.SoulRot.ID, true) > 0 and A.SummonDarkglare:GetCooldown() < 2 and (Unit(unit):HasDeBuffs(A.PhantomSingularityDebuff.ID, true) > 2 or not A.PhantomSingularity:IsTalentLearned()) then
			if PrepareDarkglare() then
				A.Toaster:SpawnByTimer("TripToast", 0, "DARKGLARE!", "PREPPING DARKGLARE", A.SummonDarkglare.ID) 				
				return true
			end
		...
		if BurstIsON(unit) and (Player:GetCovenant() == 4 or Player:GetCovenant() == 1 or Player:GetCovenant() == 0) and Unit(unit):HasDeBuffs(A.PhantomSingularityDebuff.ID, true) > 0 and Unit(unit):HasDeBuffs(A.PhantomSingularityDebuff.ID, true) < 2 then
			if PrepareDarkglare() then
				A.Toaster:SpawnByTimer("TripToast", 0, "DARKGLARE!", "PREPPING DARKGLARE", A.SummonDarkglare.ID) 				
				return true
			end
		...
		if BurstIsON(unit) and MultiUnits:GetActiveEnemies() > 2 and Player:GetCovenant() == 2 and (A.ImpendingCatastrophe:IsReady(unit) or Unit(unit):HasDeBuffs(A.ImpendingCatastrophe.ID, true) > 0) and (Unit(unit):HasDeBuffs(A.PhantomSingularityDebuff.ID, true) > 0 or not A.PhantomSingularity:IsTalentLearned()) then
			if PrepareDarkglare() then
				A.Toaster:SpawnByTimer("TripToast", 0, "DARKGLARE!", "PREPPING DARKGLARE", A.SummonDarkglare.ID) 				
				return true
			end
		...
		if BurstIsON(unit) and MultiUnits:GetActiveEnemies() > 2 and (Player:GetCovenant() == 4 or Player:GetCovenant() == 1 or Player:GetCovenant() == 0) and Unit(unit):HasDeBuffs(A.PhantomSingularityDebuff.ID, true) > 0 and Unit(unit):HasDeBuffs(A.PhantomSingularityDebuff.ID, true) < 2 then
			if PrepareDarkglare() then
				A.Toaster:SpawnByTimer("TripToast", 0, "DARKGLARE!", "PREPPING DARKGLARE", A.SummonDarkglare.ID) 					
				return true
			end
		...
		if BurstIsON(unit) and MultiUnits:GetActiveEnemies() > 2 and Player:GetCovenant() == 3 and Unit(unit):HasDeBuffs(A.SoulRot.ID, true) > 0 and A.SummonDarkglare:GetCooldown() < 2 and (Unit(unit):HasDeBuffs(A.PhantomSingularityDebuff.ID, true) > 2 or not A.PhantomSingularity:IsTalentLearned()) then
			if PrepareDarkglare() then
				A.Toaster:SpawnByTimer("TripToast", 0, "DARKGLARE!", "PREPPING DARKGLARE", A.SummonDarkglare.ID) 				
				return true
			end
		...
		if A.SeedofCorruption:IsReady("target", nil, nil, true) and UseAoE and (not isMoving) and MultiUnits:GetActiveEnemies() > 2 and A.SowtheSeeds:IsTalentLearned() and Temp.SeedofCorruptionDelay == 0 and Unit("target"):HasDeBuffs(A.SeedofCorruptionDebuff.ID, true) == 0 and Player:AreaTTD(40) > 5 and Unit(player):HealthPercent() >= DrainLifeHP then
			return A.SeedofCorruption:Show(icon)
		end	 			
		...
		if A.SeedofCorruption:IsReady("target", nil, nil, true) and UseAoE and (not isMoving) and MultiUnits:GetActiveEnemies() > 2 and A.SiphonLife:IsTalentLearned() and Temp.SeedofCorruptionDelay == 0 and CorruptionRefreshable and Unit("target"):HasDeBuffs(A.SeedofCorruptionDebuff.ID, true) == 0 and Player:AreaTTD(40) > 5 and Unit(player):HealthPercent() >= DrainLifeHP then
			return A.SeedofCorruption:Show(icon)
		end	 			
		...
		if Player:GetCovenant() ~= 4 and UseCov then
			if CovenantCall() then
				A.Toaster:SpawnByTimer("TripToast", 0, "Covenant Call!", "Covenant!", A.SummonDarkglare.ID) 				
				return true
			end
		end
		...
		if BurstIsON(unit) and Player:GetCovenant() == 2 and (A.ImpendingCatastrophe:IsReady(unit) or Unit(unit):HasDeBuffs(A.ImpendingCatastrophe.ID, true) > 0) and A.SummonDarkglare:GetCooldown() < 2 and (Unit(unit):HasDeBuffs(A.PhantomSingularityDebuff.ID, true) > 2 or not A.PhantomSingularity:IsTalentLearned()) then
			if PrepareDarkglare() then
				A.Toaster:SpawnByTimer("TripToast", 0, "DARKGLARE!", "PREPPING DARKGLARE", A.SummonDarkglare.ID) 				
				return true
			end
		...
		if BurstIsON(unit) and (Player:GetCovenant() == 4 or Player:GetCovenant() == 1 or Player:GetCovenant() == 0) and A.SummonDarkglare:GetCooldown() < 2 and (Unit(unit):HasDeBuffs(A.PhantomSingularityDebuff.ID, true) > 2 or not A.PhantomSingularity:IsTalentLearned()) then
			if PrepareDarkglare() then
				A.Toaster:SpawnByTimer("TripToast", 0, "DARKGLARE!", "PREPPING DARKGLARE", A.SummonDarkglare.ID) 				
				return true
			end
		...
		if BurstIsON(unit) and Player:GetCovenant() == 3 and (A.SoulRot:IsReady(unit) or Unit(unit):HasDeBuffs(A.SoulRot.ID, true) > 0) and A.SummonDarkglare:GetCooldown() < 2 and (Unit(unit):HasDeBuffs(A.PhantomSingularityDebuff.ID, true) > 2 or not A.PhantomSingularity:IsTalentLearned()) then
			if PrepareDarkglare() then
				A.Toaster:SpawnByTimer("TripToast", 0, "DARKGLARE!", "PREPPING DARKGLARE", A.SummonDarkglare.ID) 				
				return true
			end
		...
		if Unit(unit):HasBuffsStacks(A.ShadowEmbrace.ID, true) < (2 - num(A.ShadowBolt:IsSpellInFlight())) or Unit(unit):HasDeBuffs(A.ShadowEmbrace.ID, true) < 3 then
