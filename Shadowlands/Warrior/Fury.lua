...
A[3] = function(icon, isMulti)
	...
	--#########################
		...
		local function SingleTarget(unitID)
			...
			if A.Condemn:IsReady(unitID) and UseCovenant then
			...
			if A.DragonRoar:IsReady(player) and Unit(player):HasBuffs(A.EnrageBuff.ID, true) > 0 and MultiUnits:GetByRange(5, 2) > 1 then
		...
		if A.Rampage:IsReady(unitID) and A.Recklessness:GetCooldown() < 3 and A.RecklessAbandon:IsTalentLearned() then
			return A.Rampage:Show(icon)
		end
