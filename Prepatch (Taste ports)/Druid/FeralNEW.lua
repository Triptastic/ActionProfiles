...
A[3] = function(icon)
	...
	local RakeRefreshable = Unit(unitID):HasDeBuffs(A.RakeDebuff.ID, true) < 4 or Unit(unitID):HasDeBuffs(A.RakeDebuff.ID, true) == 0)
	local MoonfireRefreshable = (Unit(unitID):HasDeBuffs(A.MoonfireCatDebuff.ID, true) < 4 or Unit(unitID):HasDeBuffs(A.MoonfireCatDebuff.ID, true) == 0)
	local ThrashRefreshable = (Unit(unitID):HasDeBuffs(A.ThrashCat.ID, true) < 4 or Unit(unitID):HasDeBuffs(A.ThrashCatDebuff.ID, true) == 0)
...
--[[###############################
	...
	local function BloodtalonsRotation(unitID)
		...
		if A.ThrashCat:IsReady(unitID) and ThrashRefreshable and A.ThrashCat:GetSpellTimeSinceLastCast() > 4 then
		...
		if A.SwipeCat:IsReady(unitID) and (MultiUnits:GetByRange(8, 2) > 1) and A.SwipeCat:GetSpellTimeSinceLastCast() > 4 then
		...
		if A.SwipeCat:IsReady(unitID) and A.SwipeCat:GetSpellTimeSinceLastCast() > 4 then
		return A.SwipeCat:Show(icon)
		end
		
		--actions.bloodtalons+=/thrash_cat,if=buff.bt_thrash.down
		if A.ThrashCat:IsReady(unitID) and A.ThrashCat:GetSpellTimeSinceLastCast() > 4 then
		return A.ThrashCat:Show(icon)
		end
	...
	local function Cooldowns(unitID)
		...
		if A.Berserk:IsReady(unitID) then
		return A.Berserk:Show(icon)
		end
		
		--actions.cooldown+=/incarnation
		if A.Incarnation:IsReady(unitID) then
		...
		if A.TigersFury:IsReady(unitID) and (Player:EnergyDeficit() > 55 or (Unit(player):HasBuffs(A.BerserkBuff.ID, true) < 13 or Unit(player):HasBuffs(A.IncarnationBuff.ID, true) < 13)) then
		return A.TigersFury:Show(icon)
		end 
				

		--actions.cooldown+=/berserking,if=buff.tigers_fury.up|buff.bs_inc.up
		if A.Berserking:IsReady(unitID) and (Unit(player):HasBuffs(A.TigersFuryBuff.ID, true) > 0 or (Unit(player):HasBuffs(A.BerserkBuff.ID, true) > 0 or Unit(player):HasBuffs(A.IncarnationBuff.ID, true) > 0)) then
		return A.Berserking:Show(icon)
		end
		
		--actions.cooldown+=/ravenous_frenzy,if=buff.bs_inc.up|fight_remains<21
		if A.RavenousFrenzy:IsReady(unitID) and (Unit(player):HasBuffs(A.BerserkBuff.ID, true) > 0 or Unit(player):HasBuffs(A.IncarnationBuff.ID, true) > 0) or (Unit(unitID):IsBoss() and Unit(unitID):TimeToDie() < 21) then
		...
		if A.ConvoketheSpirits:IsReady(unitID) and (Unit(unitID):HasDeBuffs(A.RipDebuff.ID, true) > 4 and (Unit(player):HasBuffs(A.TigersFuryBuff.ID, true) == 0 or Unit(player):HasBuffs(A.TigersFuryBuff.ID, true) < 4) and Player:ComboPoints() == 0 and Unit(unitID):HasDeBuffs(A.ThrashCatDebuff.ID, true) > 0 and Unit(unitID):HasDeBuffs(A.RakeDebuff.ID, true) > 0) or (Unit(unitID):IsBoss() and Unit(unitID):TimeToDie() < 5) then
		...
		if A.KindredSpirits:IsReady(unitID) and Unit(player):HasBuffs(A.TigersFuryBuff.ID, true) > 0 then --or A.DeepAllegiance:IsSoulbindLearned()) then
		...
		if A.AdaptiveSwarm:IsReady(unitID) and Player:ComboPoints() > 4 then
